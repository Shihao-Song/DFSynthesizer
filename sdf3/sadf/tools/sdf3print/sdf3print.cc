...
void helpMessage(ostream &out)
{
    ...
    out << "       poosl[(settings(<file>),log(<file>))]" << endl;  // Actually: poosl[(settings(<file>),log(<file>),!p4r,pert)]
}
...
CNode *loadApplicationGraphFromFile(CString &file, CString module)
{
    ...
    if (CGetAttribute(sdf3Node, "type") != module)
        throw CException("Root element in file '" + file + "' is not of type '" + module + "'.");
 ...
 */
void SADF_PrintGraph(SADF_Graph *SADF, CPairs &format, ostream &out)
{
    ...
    else if (format.front().key == "html")
    {
        ...
        SADF2HTML(SADF, false, dot, target, url, out);
    ...
    else if (format.front().key == "php")
    {
        ...
        SADF2HTML(SADF, true, dot, target, url, out);
    ...
    else if (format.front().key == "poosl")
    {
        ...
        // The "pert" and "p4r" options are a hidden
        ...
        SADF_SimulationSettings *Settings = new SADF_SimulationSettings(SADF);
        ...
        if (SettingsFile != "")
        {
            ...
            Settings = SADF_ParseSimulationSettings(SADF, Settings, SettingsNode);
        }

        SADF2POOSL(SADF, Settings, LogFile, out, PERT, p4r);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        SADF = SADF_ConstructGraph(settings.xmlAppGraph, 0);

        // Perform the actual function
        SADF_PrintGraph(SADF, settings.arguments, out);
